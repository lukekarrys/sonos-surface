...
At topology refresh, each config key must match exactly one discovered room across the entire snapshot, including ineligible players. Missing, invalid, or ambiguous IDs fail resolution and report an error. Grouped, bonded, invisible, addressless, or otherwise ineligible targets fail as unavailable. Valid entries continue; discovered but unconfigured rooms never enter selection. Only selectable UUIDs receive resolved room policies. There is no separate policy map capable of enrolling another room. If no configured room resolves to an eligible target, playback is blocked. When discovery itself succeeds, missing or ineligible configured rooms keep Sonos health Ready and normal ten-second polling, with the specific room warning preserved. They do not discard the discovery host or a healthy topology subscription. The same applies when an accepted request’s frozen UUID is no longer eligible; only discovery failures or transport failures against a usable target enter recovery backoff. Answered HTTP/SOAP faults and UUID mismatches are ordinary job failures and leave Sonos health unchanged.

Resolution is `config display ID → discovered room → UUID`. A rename intentionally breaks the old binding until configuration is edited. UUIDs are internal identities, never human config keys. Speaker addresses come from SSDP and live topology. A successfully discovered player address may be reused in memory to read fresh topology; it is never configured or persisted. Failed topology reads trigger SSDP again. If discovery fails, retained room bindings and observations remain displayable as stale, while runtime authority is revoked and playback is blocked until fresh discovery/reconciliation. Automatic recovery is defined in [runtime lifecycles](runtime-lifecycles.md).

Selection sorts by case-insensitive human name then UUID. A room switch persists its display ID in `surface/preferred-id`. After loading configuration at boot, the runtime clears the stored and in-memory preference if its ID is no longer a key in `rooms`; selection then uses the first valid configured room without a preferred-room warning. A still-configured preference is retained even when temporarily missing or ineligible: boot selects it if eligible, otherwise warns and uses the first valid configured room without overwriting the preference. The same fallback applies when the selection disappears. A recovered room rejoins without stealing selection. Accepted work never follows a fallback or a later room switch.
...
Configuration validation completes before NVS replacement; invalid updates retain the prior config. Successful replacement advances the persisted local revision and reboots. Busy updates reject. Acceptance validates input, room, and UI context first and freezes target UUID, resolved values, provenance, and revision before reserving and queuing work. Local validation failures create no worker job and trigger no Sonos read. Execution never reruns policy; later config or selection changes cannot alter it.
