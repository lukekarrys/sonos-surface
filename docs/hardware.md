...
An intermittent identify/reactivate failure recovered on retap; phase logging now distinguishes stages, but its cause remains unresolved. No guessed RF delay or automatic playback retry was added. NFC absent at boot does not block display, Wi-Fi, or Sonos; reconnecting Grove recovered reading without reboot in a physical test. NFC polling defers while a Stick A or B gesture is pending to preserve button sampling. M5Unified uses its 500-ms click decision window.

The Stick writer implements NTAG213/215/216 page writes with preflight inspection of the capability container, static/dynamic locks, password protection, mirroring, and a single NDEF TLV followed by a terminator. The documented NTAG213 factory prefix `01 03 A0 0C 34` is accepted and preserved; its lock bytes lie outside the user area, and its five user-memory bytes count against capacity. NDEF-at-start cards also remain supported. It uses the smaller of identified user memory and CC-advertised capacity and checks page padding before any write. It rejects other layouts/protected tags and does not format tags or modify CC, lock, password, or configuration pages. The page containing the NDEF length is first invalidated, the body is written one page per loop, and that page is committed last: page 4 for NDEF-at-start tags, page 5 with the NTAG213 factory prefix. The factory descriptor remains unchanged, including its byte sharing page 5. This limits partial-message exposure but is not an atomic-write guarantee. Normal NDEF decoding/parsing and semantic comparison are required after writing.

On `stick-s3`, NTAG213 physical writing and read-back verification are verified for raw URL and compact `ss1` cards, including normal playback parsing after writing. Safari writer-page operation and the iOS/iPadOS Shortcut → unarmed draft → populated editor workflow are also verified.

The write paths follow the pinned M5 driver and [NXP memory layout](https://www.nxp.com/docs/en/data-sheet/NTAG213_215_216.pdf). NTAG213 has a measured 144 user bytes: raw URL is the smallest encoding, compact `ss1` carries explicit shuffle/repeat, and structured JSON handles advanced fields/metadata. Long URLs or advanced cards can exceed capacity. The [writer capacity table](tag-writer.md#capacity) includes exact NDEF/TLV sizes and headroom; capacity is preflighted before writing. Typical NTAG215/216 CC values advertise 496/872 bytes within 504/888 user bytes, but physical writing on those families remains unverified.

Interrupted-write recovery, power loss or card removal mid-write, and unusual protected/locked tags beyond the implemented rejection checks remain physically unverified. Existing URL cards remain readable without rewriting. NFC-B/F/V and multi-record formats remain unsupported.
...
LVGL 9.5 narrowed that finding on the same V2 board. LVGL's partial-mode flushes (whole invalidated widget areas at arbitrary offsets, through the pinned driver's unaligned address windows) rendered 1 px borders, 1 px lines at odd offsets, slider tracks, and small text intact, with and without even x/y/w/h alignment: the original loss came from one-pixel primitive windows, not from window position. Partial windows tear visibly on any motion, however, and even alignment does not change that: the panel is wired over QSPI only, no tearing-effect line reaches a GPIO, the vendor BSP never references one, and the pinned driver leaves the CO5300 TE output off, so nothing can synchronize a write to the panel's scan. The full-frame transfer is therefore required for motion, not only for thin primitives; LVGL runs in DIRECT mode into the PSRAM canvas with one ~44 ms flush per refresh, and the owner saw no tearing that way. Measured with LVGL on the main task: touch-to-flush during a drag median 17 ms / p95 21 ms in partial mode against 52 / 53 ms in direct mode; maximum main-loop poll gap under a continuous drag 77–78 ms in every mode; a continuous 33 fps animation costs about 4 ms of software rendering per frame; LVGL's software renderer needs more than the 8 KiB Arduino loop stack (about 7.5 KB of a 16 KiB stack was in use). A locally projected playback clock costs one full flush per visible change: at one repaint a second (Now Playing) the maximum main-loop poll gap stayed 52–65 ms, and at four a second (the Playground lab) 64–79 ms with about 0.9 s of flushing per 5 s, so a clock that ticks faster than its displayed resolution is pure flush cost.

Physical target sizes on this unit with the saved calibration: 44 px squares were reliable, 32 px marginal, and 24 px unreliable, worse near the edges, while injected calibrated taps hit the 24 px squares 24/24. The limit is finger contact plus the ~9 px calibration residual, not hit-testing. Widgets and gesture starts must lie inside the calibrated reachable area below and the visible corner mask: a slider whose end sits at x=334 cannot reach its maximum, and a flick that starts below y≈415 is dropped as an off-screen sample.
...
Arduino's main loop has an 8,192-byte stack on stick-s3; ws-1.8 raises it to 16 KiB because LVGL's software renderer runs on that task (about 7.7 KB in use). Build lifecycle-status JSON incrementally: a large nested initializer can inflate the inlined USB command handler's frame enough to overflow when touch-calibration serialization also formats floating-point values. The Sonos worker has a separate 24,576-byte stack.
...
BtnB resolves once after M5Unified finalizes the click count, strictly more than 500 ms after the last debounced short release while released. Holds and counts above three have no music action. Smart previous uses observed position without extrapolation: restart requires known seekability, track identity, and a same-room snapshot no older than the nominal 10-second polling interval. Unknown/stale position or seekability falls back to native Previous through the shared executor; unsupported actions retain its normal error behavior. Generic intent `previous` remains native Previous.
...
| --- | --- | --- |
| Active, normal or busy | Short press (on release): next screen (Now Playing → Grouping → Playground → Now Playing); press/hold resets inactivity. A hold longer than one second performs nothing. | Short press: no application action; sensed press resets inactivity. Hold about six seconds: board power-off. |
| Active, raw touch calibration or display-edge diagnostic | Raw touch-diagnostic build: no screen action; press/hold resets inactivity. Display-edge diagnostic: screens still cycle underneath, but the diagnostic owns the panel until `display-edge off`. | Same PMIC behavior; no diagnostic-specific action. |
...
| Sleeping (automatic deep sleep) | Press: fresh boot/reconciliation onto Now Playing. | Short press: no CPU wake. Long press: PMIC power-off; release, then press to power on. |
| Booting/waking | Release promptly. The press that woke or booted the device is consumed until a release is seen, so it never navigates; the first screen is always Now Playing. Holding during power-on/reset selects ROM download mode. | Release after power-on; a prolonged hold can power off again. |
...
Native USB needs the repository's quiet serial helper: explicit DTR/RTS writes on open caused resets even when both states were preset false. Avoiding those ioctls preserved uptime across reopen tests on this Mac. The serialport wrapper disables hangup-on-close and never explicitly sets the modem lines. Other host drivers or monitors may behave differently. USB RX is 8 KiB; its former 256-byte default lost configuration commands. USB TX uses a 16 KiB buffer and zero per-write timeout during normal operation. Routine diagnostics are best-effort: they skip lines when there is insufficient buffer space instead of delaying the Sonos worker or button sampling. The pinned HWCDC implementation can retry its configured timeout twenty times per write; a nominal 20-ms timeout therefore permits roughly 400 ms of backpressure per log write when a connected host stops reading. Serial logging stays outside the UI state mutex. Explicit configuration/status replies may yield for up to 250 ms waiting for space; only an imminent reboot restores a nonzero TX timeout to drain its acknowledgment. A disconnected or unread console may lose diagnostics, but cannot impose repeated TX waits on normal operation.

A capture can also stop receiving while the application keeps running: on ws-1.8, host reception stopped mid-capture while the board kept polling and executing USB commands, and reopening the port delivered the device's buffered backlog. The Sonos worker task serves the topology listener on port 1401, so a TCP answer there (412 for a malformed request) shows the worker is alive when the console is silent; USB `reboot` still reaching the board shows the main loop is. Long unattended captures should reopen a port that stays silent past the five-second heartbeat.
...
Wi-Fi modem power save stays at the SDK default (minimum modem sleep) while awake. Measured on both boards against one room with a ten-request poll, in paired captures of 64–69 automatic polls each without concurrent mutations: median request latency was 37–39 ms either way, poll duration median/p95/max was 413/471/566 ms (Stick) and 440/506/555 ms (ws-1.8) with power save on versus 407/521/680 ms and 433/481/537 ms with it off, and neither condition had a transport failure. Slow tails seen earlier with power save on (poll p95 up to 1.6 s) coincided with mutation traffic from the other board, not with power save. USB `wifi-sleep on|off|default` remains a non-persisted diagnostic for repeating the comparison.
