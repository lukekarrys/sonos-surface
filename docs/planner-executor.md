...
The runtime has one Sonos task and one admitted active job with an identity, deadline, and origin: one user job at a time; automatic reads yield to user input. Input during a user job, and config changes during a user job, reject rather than replaying later. Input during an automatic read preempts that read, which publishes nothing and changes no observation or health. Expired or preempted work cannot publish or dispatch; a new job may wait in the queue while the old task unwinds cooperatively. See [runtime lifecycles](runtime-lifecycles.md) for bounds and recovery. A per-UUID Application session retains observations and uncertain-command recovery state. Fresh topology before work must still contain the bound selectable UUID; if it is unavailable, fail instead of using the new selected room. Sessions update addresses only for their existing UUID.
...
An uncertain result blocks further mutations for that UUID even after cycling away and back. Ordinary refresh does not clear this requirement or replay work. Automatic authoritative reconciliation retires prepared work and verifies fresh same-UUID playback and independent topology. Successful reconciliation permits a newly requested command while retaining the previous uncertain outcome; failed reconciliation retains the block and stale observation. Timed-out/cancelled job results are retired before session reuse so a later read cannot resurrect a rejected late success. Recovery never resubmits the previous command. Durable request-ID deduplication and cross-device serialization are outside the current contract.
...
Connect/HTTP inactivity timeouts are 3/8 seconds, with a 60-second dispatch budget, 10-second readiness/verification windows, and 200-ms condition polls. The independent runtime job deadline also fences socket progress, dispatch, and publication; slow headers or chunks cannot extend it indefinitely. In-flight connect cleanup can extend cancellation by its bounded connect wait. These are experimental adapter bounds, not intent semantics or end-to-end latency guarantees.

## Observation and reconciliation

AppState keeps request `status`/`detail` and policy provenance separate from `observed` playback and `refreshError`. A failed read retains the last same-room snapshot as stale. Successful reads clear the read error while retaining command outcomes. Refresh/verification retain an existing uncertainty block; explicit authoritative reconciliation clears it for a new request. UI previews never become observed facts.

Visible playback state has four layers. `PlaybackState` (`AppState.observed`) holds authoritative Sonos facts. `PendingState` (`AppState.pending`) holds the optimistic values of one accepted user mutation: only the frozen intent's explicit transport (Play/Pause), absolute volume, seek position, shuffle, and repeat, keyed to the worker job ID; source, queue-item, and skip requests only mark content pending, and metadata, artwork, duration, and queue contents are never fabricated. `InteractionState` is the physical gesture currently owning a field and belongs to the device UI. `deriveViewModel(observed, pending, interaction, now)` in `SurfaceCore/ViewModel.h` computes the visible state per field with precedence interaction > pending > observed > unknown, so a pending or touched field never freezes any other field.

`RuntimeCoordinator::enqueueJob` creates the pending entry when it accepts the job; local rejections (busy, invalid input, room changed, recovery, queue unavailable) create none, and a toggle, which resolves its command on the worker, has none. The entry ends with that job's terminal outcome: success (its verify/refresh publication is the confirmation), failure, timeout, network loss, shutdown, or uncertainty, which keeps `recoveryRequired`. A completion for another job ID never touches it; there is no separate pending timer, so the mutation job budget bounds it. Session publication preserves it, and room selection clears it with the observation. Callers take a snapshot under the state mutex and derive outside it. Playing position advances from the observation's position anchor only while the visible transport is Playing and the observation is fresh; a stale observation holds its position, and a 60-second projection horizon backstops an observation that is neither refreshed nor marked stale.
...
The runtime reads at boot/reconnect, after commands, at a nominal ten-second cadence, and after topology invalidation. GENA on port 1401 only invalidates topology; a fresh full snapshot supplies room decisions. The independent subscription lifecycle honors the granted lifetime, rejects stale SID/lease results, and retries failed subscriptions while polling continues. Full AVTransport, RenderingControl, and ContentDirectory event subscriptions are deferred. Observations never reapply old intents. Busy execution/network delays can extend the polling interval.
