...
An uncertain result blocks further mutations for that UUID even after cycling away and back. Ordinary refresh does not clear this requirement or replay work. Recovery currently requires inspection and a deliberate restart/new request; reboot refreshes state without replaying the previous command. Cancellation, durable request-ID deduplication, automatic recovery, and cross-device serialization are deferred.
