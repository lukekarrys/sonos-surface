...
Boot reads existing Sonos state without input. Expect `SONOS_MODE`, `device-config`, room resolution, and state logs. Playback polls nominally every ten seconds; topology events invalidate discovery. One user job runs at a time; automatic reads yield to user input. Input during another user action rejects as busy. An interactive monitor forwards newline-terminated commands; Ctrl-C closes it.
...
| `ui-screen now\|rooms\|queue\|grouping\|playground` | ws-1.8: show a top-level screen or a Now Playing sub-view without touch or playback intent |
| `ui-state` | ws-1.8: print the current UI model, last sample, and frame costs as one JSON line |
| `ui-touch X Y [fingers]`, `ui-touch release` | ws-1.8: queue one injected sample in screen coordinates (0–367 by 0–447) |
| `ui-button boot` | ws-1.8: one injected debounced BOOT short press (next screen) |
| `ui-nav next` | ws-1.8: the BOOT short-press action (next screen) without the button; never local activity |
| `wifi-sleep on\|off\|default` | Development diagnostic: override Wi-Fi modem power save now and after each connection until reboot; never persisted |
| `reboot` | Reboot unless a user job is running; a running automatic read is preempted |
...
Waveshare uses release-to-submit transport/mode/volume/seek controls and four-item queue pages on its Now Playing screen; BOOT cycles Now Playing, Grouping, and Playground. USB `ui-screen now|rooms|queue|grouping|playground` shows a screen or sub-view without touch or playback, and `ui-state` prints the current model. `ui-touch`/`ui-button`/`ui-nav` inject input and are absent from the raw touch-diagnostic build. Hardware diagnostics include `touch-calibration`, `touch-calibration {JSON}`, `peripherals-retry`, `display-edge N [R]`, and `display-edge off`. See the [frontend contract](docs/waveshare-frontend.md).

Logs stream over USB only; there is no stored history after unplugging. For a bounded capture use `node --run monitor -- TARGET --port PORT --seconds 30`, redirecting output to a private `.local` log. `--until TOKEN` ends the capture early at the first line containing TOKEN, still inside the `--seconds` bound, and `--stats` ends it with one summary line: the device time the capture spans, heartbeat count, `busy=1` (user job) and `background=1` (automatic read) heartbeat ratios, worker transition count, job count, median and maximum job duration, the ratio of summed job time to the span, preempted job count, and the maximum button and UI polling gaps. Heartbeats sample a fixed cadence that can phase-lock with polling, so prefer the running ratio for worker occupancy. Durations come from the device's own log timestamps, so a buffered serial backlog does not distort them. An interactive monitor forwards typed commands; when stdin is not a terminal it reads none and requires `--seconds` so an unattended capture always ends. Live laptop capture is needed for battery tests.

**Autonomous device verification.** `node --run usb -- --port PORT --command "lifecycle-status" [--expect "lifecycles "] [--seconds N] [--lines M]` sends one command and prints the device lines that follow, without an interactive terminal. It exits zero only when the expected line arrives and nonzero on timeout, serial failure, or a device rejection token. Without `--expect`, the reply is the first line that is not background output: heartbeats, worker transitions, `request=` and Sonos job logging, and bracketed adapter or `[ui]` diagnostics are skipped, so a command whose reply is itself bracketed, such as `ui-screen`, needs an explicit `--expect`. It refuses `config` and `read-only`, which carry credentials or reboot the device and belong to `configure`/`flash`, and it never repeats command arguments in errors.

`node --run ui -- --port PORT tap X Y | drag X1 Y1 X2 Y2 [--steps N] [--interval-ms 30] | release | button boot | nav next | screen now|rooms|queue|grouping|playground | state` drives the ws-1.8 frontend without a finger, printing the device's `[ui]` lines and exiting nonzero on rejection or when a queued sample is never reported as delivered, for example because a physical finger cancelled it. Touch actions also print what the device logs for `--tail-seconds` (4 by default) after the release, which is where the release action, the admitted intent, and its job appear. Injected samples are already calibrated screen coordinates: they replace one hardware sample per 30 ms touch poll and are otherwise exactly a finger. They are queued (32 entries; overflow rejects), consumed in order, cancelled by a physical finger, and subject to the same release-after-boot rule. An open gesture holds the screen between samples for up to 1.5 seconds, so a slow host command does not read as a lifted finger. **Injected input is never local activity, never postpones sleep, and bypasses neither admission, `read_only`, nor policy**; with the committed `read_only=false` profile an injected tap mutates the configured room for real. The heartbeat's `inactivity=` field is the local-activity timer sleep uses, in seconds; it keeps counting across injected input and USB commands, which is how a capture shows that neither reset it.
...
Build variants compile the same sketch with one extra define into their own `.build/ws-1.8-<variant>` directory, so they never disturb the normal image or the device configuration; at most one variant option is accepted per build or flash. The normal ws-1.8 image is the LVGL UI described in the [frontend contract](docs/waveshare-frontend.md#screens-and-lvgl-architecture); the touch-diagnostic variant draws its raw screen without LVGL.
