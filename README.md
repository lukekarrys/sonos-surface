...
| `ui-nav next` | ws-1.8: the BOOT short-press action without the button; never local activity |
| `lvgl-render partial\|even\|direct` | ws-1.8 LVGL playground build only: switch the LVGL render strategy at runtime |
...
Build variants compile the same sketch with one extra define into their own `.build/ws-1.8-<variant>` directory, so they never disturb the normal image or the device configuration; at most one variant option is accepted per build or flash. The LVGL evaluation playground (`--lvgl-playground`, also `node --run build:ws-1.8:lvgl`, which `check:full` builds) replaces the accepted UI with LVGL test screens on top of the full runtime; see the [frontend contract](docs/waveshare-frontend.md#lvgl-playground-build).
