...
int main() {
  ...
  assert(!ui.canSeek());
  assert(slide(ui, WaveshareControl::Seek, 500, 1000).input == Input::None && !ui.seekPreview);
  ...
  assert(!ui.volumePreview && ui.touch(0, 0, 0, 2220).input == Input::None);
  ui.pressed(WaveshareControl::Volume);
  ui.slid(WaveshareControl::Volume, 50, 100);
  ...
  std::cout << "Waveshare UI checks passed: layout bounds, release intents, knob previews, "
               "navigation, bounded pages, stale/cancel/busy/read-only reconciliation, ui-state "
               "shape\n";
}
