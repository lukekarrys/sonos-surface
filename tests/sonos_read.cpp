...
public:
  ...
  surface::HttpResponse request(const std::string& path, const std::string& action,
                                const std::string& body) override {
    if (!surface::isReadOnlySonosAction(action))
      return {0, "", "Read-only diagnostic refuses mutations", true};
    ...
    curl_easy_setopt(
        curl, CURLOPT_WRITEFUNCTION, +[](char* p, size_t size, size_t count, void* data) -> size_t {
          auto& text = *static_cast<std::string*>(data);
          if (size * count > 65536 - text.size())
            return 0;
          text.append(p, size * count);
          return size * count;
        });
    ...
    if (status != CURLE_OK)
      response.error = curl_easy_strerror(status);
...
int main(int argc, char** argv) {
  ...
  auto integer = [&](int arg, uint32_t fallback, uint32_t max) {
    ...
    if (text.empty() || text.size() > 10 ||
        text.find_first_not_of("0123456789") != std::string::npos || std::stoull(text) > max) {
  ...
  for (uint32_t sample = 0; sample < samples; ++sample) {
    ...
    std::cout << "OBSERVATION "
              << Json{{"target", state.targetId},
                      {"roomDisplayId", state.roomDisplayId},
                      {"atMs", state.observedAtMs},
                      ...
                      {"positionMs", optional(state.positionMs)},
                      {"durationMs", optional(state.durationMs)},
                      {"queueBacked", optional(state.queueBacked)},
                      {"queueIndex", optional(state.queueIndex)},
                      {"queueTotal", optional(state.queueTotal)},
                      {"queueRevision", optional(state.queueRevision)},
                      {"volume", optional(state.volume)},
                      {"mute", optional(state.mute)},
                      {"shuffle", optional(state.shuffle)},
                      {"repeat",
                       state.repeat ? Json(static_cast<int>(*state.repeat)) : Json(nullptr)},
    ...
    for (const auto& item : page.items)
      std::cout << "ITEM "
                << Json{{"index", item.index},
                        ...
                        {"uri", item.uri},
                        {"durationMs", optional(item.durationMs)}}
                       .dump()
                << '\n';
