...
unsigned policyTests() {
  ...
  const Json config = {
    ...
    {"rooms", {
      ...
      {"sons-room", {{"album", {{"shuffle", true}}}, {"playlist", {{"shuffle", true}}},
                     {"track", {{"repeat", "one"}}}}}
    }}
  };
  bool readOnly = true;
  ...
  assert(roomConfigJson(selection.configured) == config["rooms"]); ++cases;
  ...
  for (int n = 0; n < 33; ++n) many["room-" + std::to_string(n)] = Json::object();
  assert(!parseDeviceRooms(Json{{"rooms", many}}, readOnly, selection.configured)); ++cases;
  ...
  for (size_t k = 0; k < urls.size(); ++k) {
    ...
    assert(defaults.shuffle == (kind == SourceKind::Album ? std::optional<bool>(false) : std::nullopt));
    assert(defaults.repeat == (kind == SourceKind::Station ? std::nullopt : std::optional<Repeat>(Repeat::Off))); ++cases;
    for (int sh = -1; sh <= 1; ++sh) for (int rp = -1; rp <= 2; ++rp) {
      auto typed = input;
      if (sh >= 0) typed.shuffle = sh == 1;
      if (rp >= 0) typed.repeat = static_cast<Repeat>(rp);
      const bool valid = k < 2 ? rp != 2 : k == 2 ? sh == -1 && rp != 1 : sh == -1 && rp == -1;
      Json modes = Json::object();
      if (sh >= 0) modes["shuffle"] = sh == 1;
      if (rp >= 0) modes["repeat"] = repeatName(static_cast<Repeat>(rp));
      Json fields = modes;
      fields["source"] = {{"service", "apple-music"}, {"url", urls[k]}};
      MusicIntent parsedInput;
      assert(validateIntent(typed).ok == valid && parseIntent(card(fields), parsedInput).ok == valid);
      ModePolicy policy;
      assert(parseModePolicy(modes, kind, policy) == valid);
      if (k < 3) {
        RoomConfig rooms;
        assert(parseDeviceRooms(Json{{"rooms", {{"test", {{sourceKindName(kind), modes}}}}}}, readOnly, rooms) == valid);
      }
      FakeSonos transport; Application app(transport, {"RINCON_A", selection.resolvedPolicies, 42});
    ...
    for (const auto& room : selection.rooms) {
      ...
      const bool shuffled = k < 2 && (room.displayId == "sons-room" || (k == 1 && room.displayId == "living-room"));
      const auto shuffle = shuffled ? std::optional<bool>(true) : defaults.shuffle;
      const auto repeat = k == 2 && room.displayId == "sons-room" ? std::optional<Repeat>(Repeat::One) : defaults.repeat;
      ...
      assert(policy["shuffle"]["origin"] == (shuffled ? "room-policy:" + room.displayId : k == 0 ? "source-default:album" : "preserve"));
      assert(policy["repeat"]["origin"] == (k == 3 ? "preserve" : k == 2 && room.displayId == "sons-room" ? "room-policy:sons-room" : std::string("source-default:") + sourceKindName(kind)));
      assert(!input.shuffle && !input.repeat && resolved.policyRevision == 42);
      assert(std::find(diagnostics["preserved"].begin(), diagnostics["preserved"].end(), "repeat") == diagnostics["preserved"].end() || k == 3);
      ++cases;
      if (k < 3) {
        if (k < 2) input.shuffle = false;
        ...
        if (k < 2) assert(explicitResult.intent.shuffle == false && shuffleOrigin(explicitResult) == "explicit");
        input.shuffle.reset(); input.repeat.reset(); ++cases;
      }
    }
  }
  // No source means no room/default inference even with an observed album.
  for (const Json& fields : std::vector<Json>{{{"repeat", "one"}}, {{"shuffle", true}}, {{"volume", {{"set", 0}}}}}) {
    ...
    assert(describeOrigin(resolved.provenance.at(PolicyField::Repeat)) == (input.repeat ? "explicit" : "preserve"));
    assert(shuffleOrigin(resolved) == (input.shuffle.has_value() ? "explicit" : "preserve")); ++cases;
  }
  ...
  for (const auto& url : std::vector<std::string>{album, playlist, album + "?i=111"}) {
    ControlHttp http; DirectSonos sonos(http, {http.id, "52231"});
    Application app(sonos, {http.id, selection.resolvedPolicies, 43});
    assert(app.submit(url).ok);
    assert(http.mode == (url == album ? "NORMAL" : "SHUFFLE_NOREPEAT")); ++cases;
  }
  {
    ControlHttp http; http.id = "RINCON_C"; http.room = "Sons Room";
    http.uri = "x-rincon-queue:RINCON_C#0";
    DirectSonos sonos(http, {http.id, "52231"});
    Application app(sonos, {http.id, selection.resolvedPolicies, 43});
    assert(app.submit(album + "?i=111").ok && http.mode == "SHUFFLE_REPEAT_ONE"); ++cases;
    assert(app.submit(card({{"source", {{"service", "apple-music"}, {"url", album + "?i=111"}}},
                            {"repeat", "off"}})).ok && http.mode == "SHUFFLE_NOREPEAT"); ++cases;
  }
  // Field precedence is independent: explicit shuffle must not suppress derived repeat.
  for (const auto& url : {album, playlist}) {
    auto input = parsed(url); input.shuffle = false;
    auto resolved = resolvePolicy(input, {"RINCON_C", selection.resolvedPolicies, 43});
    assert(resolved.intent.shuffle == false && shuffleOrigin(resolved) == "explicit");
    assert(resolved.intent.repeat == Repeat::Off);
    assert(resolved.provenance.at(PolicyField::Repeat).origin == PolicyOrigin::SourceDefault); ++cases;
    input.shuffle.reset(); input.repeat = Repeat::All;
    ...
    assert(resolved.intent.repeat == Repeat::All && resolved.provenance.at(PolicyField::Repeat).origin == PolicyOrigin::Explicit); ++cases;
  }
  ...
  assert(frozen.intent.shuffle == true && frozen.intent.repeat == Repeat::All && frozen.policyRevision == 44);
  assert(describeIntent(parsed(album), frozen) == frozenDiagnostics && app.state().provenance == frozen.provenance); ++cases;
  auto later = resolvePolicy(parsed(album), {http.id, selection.resolvedPolicies, 45});
  assert(later.intent.shuffle == false && later.intent.repeat == Repeat::Off && later.policyRevision == 45); ++cases;
  return cases;
}
