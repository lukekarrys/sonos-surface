...
int main() {
  for (unsigned spacing : {100u, 200u, 400u}) {
    ...
    for (uint32_t t = 0; t < 1500; t += 5) {
      ...
      a.setRawState(t, pressed); b.setRawState(t, false);
      const auto event = stickButtonInput(a, b);
      rooms += event == Input::RoomNext; refreshes += event == Input::Refresh;
      if (t == 180) deferred = stickButtonPending(a, b);
    }
  ...
  for (bool hold : {false, true}) {
    ...
    for (uint32_t t = 0; t < 1800; t += 5) {
      a.setRawState(t, t >= 100 && t < (hold ? 800u : 160u)); b.setRawState(t, false);
  ...
  for (uint32_t t = 0; t < 1000; t += 5) {
    a.setRawState(t, false); b.setRawState(t, t >= 100 && t < 160);
    toggles += stickButtonInput(a, b) == Input::Toggle;
  }
  assert(toggles == 1);
  std::cout << "Stick button checks passed: single/double/hold/B, deferred NFC, one action per gesture\n";
}
