...
int main() {
  ...
  const int samples[][4] = {{87, 120, 92, 140},   {307, 136, 276, 140}, {69, 289, 92, 270},
  ...
  for (const auto& c : {TouchCalibration{}, restored})
    for (const auto& p :
         {TouchPoint{-1, 0}, TouchPoint{368, 100}, TouchPoint{100, -1}, TouchPoint{100, 448}})
  ...
  for (auto pair : {std::pair<const char*, double>{"version", 2},
                    {"controller", 22},
                    {"x_scale", 0},
                    {"y_scale", 1e300},
                    {"x_offset", -113},
  ...
  invalid.xScale = std::numeric_limits<double>::quiet_NaN();
  ...
  std::cout << "Touch checks passed: persisted fit, fallback, schema/bounds, atomic rejection, "
               "other unit\n";
}
