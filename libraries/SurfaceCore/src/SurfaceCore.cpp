...
bool keys(const Json& object, std::initializer_list<const char*> allowed) {
  ...
  for (auto it = object.begin(); it != object.end(); ++it) {
    if (std::none_of(allowed.begin(), allowed.end(),
                     [&](const char* key) { return key == it.key(); }))
      return false;
  }
  return true;
}
...
Result normalizeAppleUrl(const std::string& input, Source& source) {
  ...
  const auto bad = Result::fail(
      "Unsupported Apple Music URL: use an album, playlist, track, or station share URL");
  if (url.size() > 4096 || !validUtf8(url) || url.compare(0, 8, "https://") != 0)
    return bad;
  ...
  if (host != "music.apple.com" && host != "music.apple.com:443")
    return bad;
  auto fragment = url.find('#', slash);
  std::string pathQuery =
      url.substr(slash + 1, fragment == std::string::npos ? fragment : fragment - slash - 1);
  auto question = pathQuery.find('?');
  auto path = pathQuery.substr(0, question);
  auto parts = split(path, '/');
  if (parts.size() != 4 || parts[0].empty() || parts[2].empty() || parts[3].empty())
    return bad;
  Source parsed;
  if (!decode(parts[0], parsed.storefront) || !decode(parts[3], parsed.catalogId))
    return bad;
  std::string slug;
  if (!decode(parts[2], slug))
    return bad;
  if (parts[1] == "album") {
    if (!digits(parsed.catalogId))
      return bad;
    parsed.kind = SourceKind::Album;
  } else if (parts[1] == "playlist")
    parsed.kind = SourceKind::Playlist;
  else if (parts[1] == "station") {
    if (parsed.catalogId.compare(0, 3, "ra.") != 0 || parsed.catalogId.size() <= 3 ||
        parsed.catalogId.find_first_not_of(
  ...
  if (question != std::string::npos) {
    for (const auto& pair : split(pathQuery.substr(question + 1), '&')) {
      ...
      if (!decode(pair.substr(0, equals), name) ||
          !decode(equals == std::string::npos ? "" : pair.substr(equals + 1), value))
        return bad;
      if (name == "i") {
        if (hasTrack || parts[1] != "album" || !digits(value))
          return bad;
        hasTrack = true;
        trackId = value;
      }
...
Result validateIntent(const MusicIntent& intent) {
  if (!intent.source && !intent.transport && !intent.shuffle.has_value() && !intent.repeat &&
      !intent.volume && !intent.seekPositionMs && !intent.queueIndex)
    return Result::fail("Empty intent");
  ...
  if ((intent.seekPositionMs || intent.queueIndex) &&
      (intent.source || intent.transport == TransportCommand::Next ||
  ...
  if (intent.transport && *intent.transport != TransportCommand::Play &&
      *intent.transport != TransportCommand::Pause && *intent.transport != TransportCommand::Next &&
      *intent.transport != TransportCommand::Previous)
    return Result::fail("Unsupported transport");
  if (intent.volume &&
      (intent.volume->value > 100 || intent.volume->value < (intent.volume->relative ? -100 : 0)))
    return Result::fail("Invalid volume range");
  if (intent.repeat && *intent.repeat != Repeat::Off && *intent.repeat != Repeat::All &&
      *intent.repeat != Repeat::One)
    return Result::fail("Invalid repeat");
  if ((intent.transport == TransportCommand::Next ||
       intent.transport == TransportCommand::Previous) &&
      (intent.source || intent.shuffle.has_value() || intent.repeat))
    return Result::fail("Next/previous cannot combine with source or mode");
  if (intent.source) {
    ...
    if (!r.ok)
      return r;
    if (normalized.catalogId != intent.source->catalogId ||
        normalized.kind != intent.source->kind ||
        normalized.storefront != intent.source->storefront)
      return Result::fail("Inconsistent normalized source");
  }
  return {};
}
...
Result parseIntent(const std::string& payload, MusicIntent& output) {
  ...
  if (text.compare(0, 8, "https://") == 0) {
    ...
    if (!boundedJson(text))
      return Result::fail("Invalid JSON or nesting exceeds eight containers");
    ...
    auto root = Json::parse(
        ...
        [&](int, Json::parse_event_t event, Json& value) {
          if (event == Json::parse_event_t::object_start)
            keyStack.emplace_back();
          if (event == Json::parse_event_t::key &&
              !keyStack.back().insert(value.get<std::string>()).second)
            duplicate = true;
        ...
        false);
    if (root.is_discarded() || duplicate || members(root) > 128)
      return Result::fail("Invalid/duplicate/oversized JSON");
    if (!keys(root, {"format", "version", "intent", "label", "extensions", "requires"}) ||
        ...
        !root.contains("intent"))
      return Result::fail("Expected sonos-surface version 1 envelope");
    if (root.contains("label") && !root["label"].is_string())
      return Result::fail("label must be text");
    if (root.contains("extensions")) {
      if (!root["extensions"].is_object())
        return Result::fail("extensions must be an object");
      for (auto it = root["extensions"].begin(); it != root["extensions"].end(); ++it)
        if (it.key().find('.') == std::string::npos)
          return Result::fail("Extension names must be namespaced");
    }
    ...
    if (fields.contains("seek")) {
      ...
      if (!keys(seek, {"positionMs"}) || seek.size() != 1 || !seek.contains("positionMs") ||
          !seek["positionMs"].is_number_integer() || seek["positionMs"] < 0 ||
          seek["positionMs"] > UINT32_MAX)
    ...
    if (fields.contains("queueIndex")) {
      ...
      if (!index.is_number_integer() || index < 0 || index >= UINT32_MAX)
        return Result::fail("Invalid queueIndex");
    ...
    if (fields.contains("source")) {
      ...
      if (!keys(s, {"service", "url"}) || !s.contains("service") || s["service"] != "apple-music" ||
          !s.contains("url") || !s["url"].is_string())
    ...
    if (fields.contains("transport")) {
      if (fields["transport"] == "play")
        intent.transport = TransportCommand::Play;
      else if (fields["transport"] == "pause")
        intent.transport = TransportCommand::Pause;
      else if (fields["transport"] == "next")
        intent.transport = TransportCommand::Next;
      else if (fields["transport"] == "previous")
        intent.transport = TransportCommand::Previous;
      else
        return Result::fail("Unsupported transport");
    }
    if (fields.contains("repeat")) {
      if (fields["repeat"] == "off")
        intent.repeat = Repeat::Off;
      else if (fields["repeat"] == "all")
        intent.repeat = Repeat::All;
      else if (fields["repeat"] == "one")
        intent.repeat = Repeat::One;
      else
        return Result::fail("repeat must be off/all/one");
    }
    if (fields.contains("volume")) {
      const auto& v = fields["volume"];
      if (!keys(v, {"set", "delta"}) || v.size() != 1)
        return Result::fail("volume requires exactly one set/delta");
      const auto& n = v.begin().value();
      if (!n.is_number_integer() || n < -100 || n > 100)
        return Result::fail("Invalid volume integer");
      intent.volume = Volume{v.contains("delta"), n.get<int>()};
    }
    if (fields.contains("shuffle")) {
      if (!fields["shuffle"].is_boolean())
        return Result::fail("shuffle must be true or false");
      intent.shuffle = fields["shuffle"].get<bool>();
    }
  }
  auto result = validateIntent(intent);
  if (result.ok)
    output = std::move(intent);
  return result;
}
...
Result validateSourceModes(SourceKind kind, const ModePolicy& modes) {
  if (modes.repeat && *modes.repeat != Repeat::Off && *modes.repeat != Repeat::All &&
      *modes.repeat != Repeat::One)
    return Result::fail("Invalid repeat");
  switch (kind) {
  case SourceKind::Album:
  case SourceKind::Playlist:
    if (modes.repeat == Repeat::One)
      return Result::fail("Repeat one is invalid for album/playlist sources");
    break;
  case SourceKind::Track:
    if (modes.shuffle.has_value())
      return Result::fail("Shuffle is unsupported for tracks");
    if (modes.repeat == Repeat::All)
      return Result::fail("Repeat all is invalid for tracks");
    break;
  case SourceKind::Station:
    if (modes.shuffle.has_value() || modes.repeat)
      return Result::fail("Shuffle/repeat are unsupported for stations");
    break;
  default:
    return Result::fail("Invalid source kind");
  }
  return {};
}
ModePolicy sourceDefaults(SourceKind kind) {
  switch (kind) {
  case SourceKind::Album:
    return {false, Repeat::Off};
  case SourceKind::Playlist:
  case SourceKind::Track:
    return {std::nullopt, Repeat::Off};
  case SourceKind::Station:
    return {};
  }
  return {};
...
std::string describeOrigin(const FieldProvenance& provenance) {
  ...
  case PolicyOrigin::Preserved:
    return "preserve";
  case PolicyOrigin::Explicit:
    return "explicit";
...
Result decodeNdefText(const uint8_t* data, size_t size, std::string& text) {
  if (!data || size < 2 || size > 4160 || (data[0] & 0xC0))
    return Result::fail("Expected UTF-8 NDEF Text payload");
  size_t language = data[0] & 0x3F;
  if (!language || language + 1 >= size || size - language - 1 > 4096)
    return Result::fail("Invalid NDEF language/length");
  for (size_t i = 1; i <= language; ++i)
    if (!std::isalnum(data[i]) && data[i] != '-')
      return Result::fail("Invalid NDEF language");
  text.assign(reinterpret_cast<const char*>(data + 1 + language), size - 1 - language);
  if (!validUtf8(text))
    return Result::fail("Malformed UTF-8 NDEF text");
  return {};
}

Result decodeNdefUri(const uint8_t* data, size_t size, std::string& url) {
  if (!data || size < 2 || size > 4097)
    return Result::fail("Invalid NDEF URI length");
  ...
  if (data[0] != 0x00 && data[0] != 0x04)
    return Result::fail("Unsupported NDEF URI prefix (need Apple Music HTTPS URL)");
  std::string decoded = data[0] == 0x04 ? "https://" : "";
  if (size - 1 > 4096 - decoded.size())
    return Result::fail("NDEF URI exceeds 4096 bytes");
  decoded.append(reinterpret_cast<const char*>(data + 1), size - 1);
  if (!validUtf8(decoded))
    return Result::fail("Malformed UTF-8 NDEF URI");
  Source source;
...
Result decodeNdefRecord(uint8_t tnf, const std::string& type, const uint8_t* data, size_t size,
                        std::string& text) {
  if (tnf == 0 && type.empty() && !size)
    return Result::fail("Empty NDEF record: no music URL");
  if (tnf != 1)
    return Result::fail("Unsupported NFC TNF=" + std::to_string(tnf));
  if (type == "T")
    return decodeNdefText(data, size, text);
  if (type == "U")
    return decodeNdefUri(data, size, text);
  ...
  if (!type.empty())
    return Result::fail("Unsupported NFC type=" + type.substr(0, 32));
  if (!data || !size || size > 4096)
    return Result::fail("Invalid empty-type NFC URL length");
  Source source;
...
Result makePlan(const ResolvedIntent& resolved, Plan& plan) {
  ...
  if (!valid.ok)
    return valid;
  plan = {resolved, {}};
  if (resolved.intent.seekPositionMs)
    plan.operations.push_back(Operation::Seek);
  if (resolved.intent.queueIndex)
    plan.operations.push_back(Operation::SelectQueueItem);
  if (resolved.intent.source) {
    if (resolved.intent.source->kind == SourceKind::Station) {
      if (resolved.intent.transport != TransportCommand::Play)
        plan.operations.push_back(Operation::Stop);
      plan.operations.push_back(Operation::SelectStation);
    } else
      plan.operations = {Operation::Stop, Operation::ClearQueue, Operation::AddSource,
                         Operation::SelectQueue, Operation::ApplyMode};
  } else if (resolved.intent.shuffle.has_value() || resolved.intent.repeat)
    plan.operations.push_back(Operation::ApplyMode);
  if (resolved.intent.volume)
    plan.operations.push_back(Operation::SetVolume);
  if (resolved.intent.transport) {
    switch (*resolved.intent.transport) {
    case TransportCommand::Play:
      plan.operations.push_back(Operation::Play);
      break;
    case TransportCommand::Pause:
      plan.operations.push_back(Operation::Pause);
      break;
    case TransportCommand::Next:
      plan.operations.push_back(Operation::Next);
      break;
    case TransportCommand::Previous:
      plan.operations.push_back(Operation::Previous);
      break;
    }
  } else if (resolved.intent.source)
    plan.operations.push_back(Operation::RestoreTransport);
  return {};
}
...
bool selectObservedRoom(AppState& state, const Room& room) {
  if (state.observed.targetId == room.id)
    return false;
  state = {};
...
bool publishSelectedState(AppState& state, const AppState& incoming,
...
Result Application::queue(uint32_t start, uint32_t count) {
  ...
  if (result.ok && page.targetId != context_.targetId)
    result = Result::fail("Queue target mismatch");
  if (result.ok) {
    if (state_.observed.queueRevision != page.revision) {
      state_.observed.stale = true;
      state_.observed.queueIndex.reset();
    }
    if (state_.observed.queueBacked == true)
      state_.observed.queueTotal = page.total;
...
Result Application::submitToggle(const PolicyContext& bound) {
  ...
  if (bound.targetId.empty() || bound.targetId != context_.targetId)
    return reject("Bound toggle target mismatch");
  auto result = refresh();
  if (!result.ok)
    return result;
  ...
  if (current.playback == "PLAYING")
    intent.transport = TransportCommand::Pause;
  else if (current.playback == "PAUSED_PLAYBACK" || current.playback == "STOPPED")
    intent.transport = TransportCommand::Play;
  else
    return reject("Cannot toggle playback state: " + current.playback);
  state_.detail = "Toggle resolved: " + current.playback + " -> " +
                  (*intent.transport == TransportCommand::Play ? "play" : "pause") +
                  " target=" + bound.targetId +
...
Result Application::submit(const ResolvedIntent& accepted) {
  ...
  state_.status = result.ok          ? "succeeded"
