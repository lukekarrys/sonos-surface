...
inline bool validCalibration(const TouchCalibration& c) {
  return (c.controller == 0x15 || c.controller == 0x38) &&
         std::isfinite(c.xScale) && std::isfinite(c.yScale) &&
         std::isfinite(c.xOffset) && std::isfinite(c.yOffset) &&
...
inline bool parseCalibration(const std::string& text, TouchCalibration& output) {
  ...
  if (!j.is_object() || j.size() != 6 || !j.contains("version") ||
      !j["version"].is_number_integer() || j["version"] != 1 ||
      !j.contains("controller") || !j["controller"].is_number_integer() ||
      (j["controller"] != 21 && j["controller"] != 56)) return false;
  ...
  TouchCalibration c{j["controller"].get<int>(), j["x_scale"].get<double>(),
                     j["x_offset"].get<double>(), j["y_scale"].get<double>(), j["y_offset"].get<double>()};
  if (!validCalibration(c)) return false;
  output = c;
  return true;
}
inline std::string calibrationJson(const TouchCalibration& c) {
  return nlohmann::json{{"version", 1}, {"controller", c.controller},
    ...
    {"y_scale", c.yScale}, {"y_offset", c.yOffset}}.dump();
}
inline TouchPoint waveshareTouchPoint(int rawX, int rawY, const TouchCalibration& c = {}) {
  ...
  if (!validCalibration(c)) return {rawX, rawY};
  ...
  if (x < 0 || x >= 368 || y < 0 || y >= 448) return {-1, -1};
  return {x, y};
}
} // namespace surface::device
