...
public:
  ...
  bool select(const PlaybackState& state) {
    ...
    if (room == state.targetId && track == state.trackUri && url == nextUrl)
      return false;
    room = state.targetId;
...
inline void artworkThumbnail(const uint16_t* source, unsigned width, unsigned height,
                             uint16_t* target) {
  ...
  if (width >= height)
    h = std::max(1u, artworkSize * height / width);
  else
    w = std::max(1u, artworkSize * width / height);
  const unsigned left = (artworkSize - w) / 2, top = (artworkSize - h) / 2;
  for (unsigned y = 0; y < h; ++y)
    for (unsigned x = 0; x < w; ++x)
      target[(top + y) * artworkSize + left + x] =
          source[(uint64_t(y) * height / h) * width + uint64_t(x) * width / w];
}
} // namespace surface::device
