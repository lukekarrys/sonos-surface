...
inline Repeat nextRepeat(Repeat r) {
  return r == Repeat::Off ? Repeat::All : r == Repeat::All ? Repeat::One : Repeat::Off;
}
inline const char* repeatLabel(Repeat r) {
  return r == Repeat::Off ? "Off" : r == Repeat::All ? "All" : "One";
}
inline const char* playbackLabel(PlaybackStatus s) {
  ...
  case PlaybackStatus::NoMedia:
    return "No music";
  case PlaybackStatus::Transitioning:
...
class WaveshareUi {
  ...
  BoardEvent mutation(MusicIntent intent, const std::string& label, uint32_t now) {
    ...
    if (!fresh()) {
      notify("Refresh room first", now);
      return {};
    }
    if (state.recoveryRequired) {
      notify("Check room; recovery needed", now);
      return {};
    }
    ...
    BoardEvent event;
    event.input = Input::Intent;
    event.intent = std::move(intent);
    event.targetId = state.observed.targetId;
    event.trackIdentity = state.observed.trackUri;
    event.queueRevision = state.observed.queueRevision;
    return event;
  }
...
public:
  ...
  bool canPlay() const {
    auto s = state.observed.transport;
    return fresh() && (s == PlaybackStatus::Playing || s == PlaybackStatus::Paused ||
                       s == PlaybackStatus::Stopped);
  }
  bool activeQueue() const { return fresh() && state.observed.queueBacked == true; }
  bool matchingPage() const {
    return state.queue && state.queue->targetId == state.observed.targetId &&
           state.queue->start == queueStart;
  }
  bool canSelectQueue() const {
    return activeQueue() && matchingPage() && state.observed.queueRevision == state.queue->revision;
  }
  void notify(const std::string& text, uint32_t now) {
    toast = text;
    toastUntil = now + 3500;
    dirty = true;
  }
  ...
  void update(const AppState& next, const BoardContext& c, uint32_t now) {
    ...
    if (!roomsChanged)
      for (unsigned i = 0; i < c.rooms.size(); ++i)
        if (c.rooms[i].id != context.rooms[i].id || c.rooms[i].name != context.rooms[i].name)
          roomsChanged = true;
    const bool pageChanged = next.queue.has_value() != state.queue.has_value() ||
                             (next.queue && state.queue &&
                              (next.queue->observedAtMs != state.queue->observedAtMs ||
    ...
    if (touching &&
        (next.observed.trackUri != gestureTrack || next.observed.queueRevision != gestureRevision ||
         next.observed.durationMs != gestureDuration || next.observed.stale || !c.online))
      cancelTouch();
    // Observation replacement also drops pages; never retain an independent UI cache.
    state = next;
    context = c;
    if (queueRequested && c.busy)
      sawQueueBusy = true;
    ...
    if (matchingPage() && queueStart && queueStart >= state.queue->total) {
      queueStart = state.queue->total ? ((state.queue->total - 1) / waveshareLayout::pageSize) *
                                            waveshareLayout::pageSize
                                      : 0;
      queueRequested = false;
    }
    if (roomStart >= c.rooms.size())
      roomStart = 0;
    const auto outcome = std::to_string(next.requestId) + ":" + next.status + ":" + next.detail;
    if (outcomeInitialized && outcome != lastOutcome && next.status != "idle" &&
        next.status != "pending") {
      const bool blocked = next.detail.find("READ_ONLY") != std::string::npos;
      if (blocked)
        notify("READ ONLY - command not sent", now);
      else if (next.status == "succeeded")
        notify(c.readOnly ? "READ ONLY - no change sent" : "Updated from Sonos", now);
      else
        notify("Command failed - check room", now);
    }
    lastOutcome = outcome;
    outcomeInitialized = true;
    if (c.feedback != lastFeedback && !c.feedback.empty())
      notify(c.feedback, now);
    lastFeedback = c.feedback;
    if (!toast.empty() && int32_t(now - toastUntil) >= 0) {
      toast.clear();
      dirty = true;
    }
  }
  ...
  BoardEvent requestQueue() {
    if (screen != WaveshareScreen::Queue || context.busy || !context.online ||
      ...
      return {};
    if (matchingPage() || !state.queueError.empty())
      return {};
    queueRequested = true;
    sawQueueBusy = false;
    BoardEvent event;
    event.input = Input::QueuePage;
    event.targetId = state.observed.targetId;
    event.start = queueStart;
    event.count = waveshareLayout::pageSize;
    return event;
  }
  BoardEvent touch(int x, int y, int fingers, uint32_t now) {
    if (fingers > 1 || (fingers && (x < 0 || y < 0 || x >= 368 || y >= 448))) {
      cancelTouch();
      return {};
    }
    if (fingers) {
    ...
    case WaveshareControl::Queue:
      ...
      queueStart = state.observed.queueIndex.value_or(0) / waveshareLayout::pageSize *
                   waveshareLayout::pageSize;
      queueRequested = false;
      return requestQueue();
    ...
    case WaveshareControl::PageNext: {
      ...
      if (screen == WaveshareScreen::Rooms) {
        if (forward && uint64_t(roomStart) + 4 < context.rooms.size())
          roomStart += 4;
        if (!forward && roomStart >= 4)
          roomStart -= 4;
      } else {
        if (context.busy) {
          notify("Busy - try again", now);
        ...
        if (forward && matchingPage() && uint64_t(queueStart) + 4 < state.queue->total)
          queueStart += 4;
        else if (!forward && queueStart >= 4)
          queueStart -= 4;
        else
          return {};
        state.queue.reset();
    ...
    case WaveshareControl::Seek:
      if (!seek || !canSeek()) {
        notify("This source cannot seek", now);
        return {};
      }
      intent.seekPositionMs = *seek;
      return mutation(intent, "Seek", now);
    case WaveshareControl::Previous:
      intent.transport = TransportCommand::Previous;
      return mutation(intent, "Previous", now);
    case WaveshareControl::Next:
      intent.transport = TransportCommand::Next;
      return mutation(intent, "Next", now);
    ...
    case WaveshareControl::Row3: {
      ...
      if (screen == WaveshareScreen::Rooms && roomStart + row < context.rooms.size()) {
        ...
        screen = WaveshareScreen::NowPlaying;
        BoardEvent event;
        event.input = Input::RoomSelect;
        event.text = context.rooms[roomStart + row].displayId;
        return event;
      }
      if (screen == WaveshareScreen::Queue && matchingPage() && row < state.queue->items.size()) {
        if (!canSelectQueue()) {
          notify("Stored queue - selection unavailable", now);
          return {};
        }
    ...
    notify("Control unavailable - refresh", now);
    return {};
  }
};
} // namespace surface::device
