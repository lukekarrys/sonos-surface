...
public:
  size_t size = 0;
  bool aborted = false;
  Download(uint8_t* p, uint32_t generation, uint32_t end)
      : buffer(p), token(generation), deadline(end) {}
...
Buffer<uint16_t> load(const Request& request) {
  ...
  do {
    if (!current(request.token) || WiFi.status() != WL_CONNECTED) {
      error = "cancelled/offline";
      break;
    }
    auto input = allocate<uint8_t>(maxArtworkBytes);
    if (!input) {
      error = "download allocation";
      break;
    }
    HTTPClient http;
    ...
    if (!http.begin(request.url.c_str())) {
      error = "HTTP begin";
      break;
    }
    ...
    const auto read =
        status == 200 && expected <= int(maxArtworkBytes) ? http.writeToStream(&sink) : -1;
    http.end();
    bytes = sink.size;
    fetched = millis() - began;
    if (status != 200 || read < 0 || sink.aborted || !bytes ||
        (expected >= 0 && bytes != size_t(expected))) {
      error = "HTTP status/size/timeout/cancel";
      break;
    }
    ...
    if (bytes < 2 || input.get()[0] != 0xFF || input.get()[1] != 0xD8) {
      error = "not JPEG";
      break;
    }
    esp_jpeg_image_cfg_t cfg{};
    cfg.indata = input.get();
    ...
    if (esp_jpeg_get_image_info(&cfg, &info) != ESP_OK || !info.width || !info.height ||
      ...
      error = "JPEG dimensions/header";
      break;
    }
    width = info.width;
    height = info.height;
    unsigned scale = 0;
    while (scale < 3 && (std::max(width, height) >> (scale + 1)) >= artworkSize)
      ++scale;
    cfg.out_scale = static_cast<esp_jpeg_image_scale_t>(scale);
    if (esp_jpeg_get_image_info(&cfg, &info) != ESP_OK || !info.output_len ||
        info.output_len > 128 * 1024) {
      error = "JPEG output bound";
      break;
    }
    auto output = allocate<uint8_t>(info.output_len);
    if (!output) {
      error = "decode allocation";
      break;
    }
    cfg.outbuf = output.get();
    cfg.outbuf_size = info.output_len;
    const auto decodeStart = millis();
    if (esp_jpeg_decode(&cfg, &info) != ESP_OK || !info.width || !info.height) {
      error = "JPEG decode";
      break;
    }
    if (!current(request.token)) {
      error = "superseded during decode";
      break;
    }
    thumbnail = allocate<uint16_t>(artworkSize * artworkSize);
    if (!thumbnail) {
      error = "thumbnail allocation";
      break;
    }
    artworkThumbnail(reinterpret_cast<uint16_t*>(output.get()), info.width, info.height,
                     thumbnail.get());
    decoded = millis() - decodeStart;
    error = "ok";
    Serial.printf("[artwork] working token=%lu scaled=%ux%u heap=%lu psram-free=%lu\n",
                  request.token, info.width, info.height, ESP.getFreeHeap(), ESP.getFreePsram());
  } while (false);
  Serial.printf("[artwork] token=%lu result=%s bytes=%u source=%ux%u fetch=%lu decode=%lu "
                "total=%lu heap-before=%lu heap-after=%lu psram-before=%lu psram-after=%lu\n",
                request.token, error, unsigned(bytes), width, height, fetched, decoded,
                millis() - began, beforeHeap, ESP.getFreeHeap(), beforePsram, ESP.getFreePsram());
  return thumbnail;
}
void worker(void*) {
  for (;;) {
    ...
    if (xQueueSend(completions, &result, 0) != pdTRUE)
      heap_caps_free(result.pixels);
  }
}
bool beginWorker(uint32_t now) {
  if (started)
    return true;
  if (int32_t(now - startRetry) < 0)
    return false;
  ...
  if (requests && completions &&
      xTaskCreate(worker, "surface-art", 8192, nullptr, 1, nullptr) == pdPASS) {
    started = true;
    return true;
  }
  if (requests)
...
bool artworkUpdate(const PlaybackState& observed, bool online, uint32_t now) {
  ...
  if (started) {
    ...
    if (xQueueReceive(completions, &result, 0) == pdTRUE) {
      ...
      if (state.complete(result.token, bool(pixels), now)) {
        visible = std::move(pixels);
        changed = true;
        Serial.printf("[artwork] publish token=%lu ready=%d room=%s\n", result.token, state.ready,
                      observed.room.c_str());
      } else
        Serial.printf("[artwork] discarded stale token=%lu\n", result.token);
    }
  }
  if (!running && state.wantsLoad(online, observed.stale, now) && beginWorker(now)) {
    ...
    if (xQueueSend(requests, &request, 0) == pdTRUE)
      ...
      delete request;
      state.complete(state.generation, false, now);
    }
  }
  return changed;
}
